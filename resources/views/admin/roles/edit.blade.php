        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror"
                       ...
                       @if($role->name === 'Super-Admin' && !Auth::user()->hasRole('Super-Admin')) readonly @endif >
                @error('name') <span class="invalid-feedback">{{ $message }}</span> @enderror
                 @if($role->name === 'Super-Admin')
                    <small class="form-text text-muted">Super-Admin role name cannot be changed by non-Super-Admins.</small>
                @endif
            </div>
            ...
            <div class="form-group">
                <label>Assign Permissions</label>
                 @if($role->name === 'Super-Admin')
                    <p class="alert alert-info">The Super-Admin role automatically has all permissions. They cannot be manually changed here.</p>
                @else
...
<script>
    $(document).ready(function() {
        function checkGroupSelectAll(group) {
            var allInGroup = $('.permission-checkbox[data-group="' + group + '"]');
            if (allInGroup.length > 0) { // only if there are checkboxes in the group
        ...
        $('.permission-checkbox').on('change', function() {
            var group = $(this).data('group');
            checkGroupSelectAll(group);
        });

        // Initial check for select-all checkboxes on page load
