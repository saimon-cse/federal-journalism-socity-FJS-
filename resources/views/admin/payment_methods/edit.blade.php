...
@section('page-title')
    Edit Method: <span class="text-primary">{{ $paymentMethod->name }}</span>
@endsection
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Update Payment Method Details</h3>
        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('method_key') is-invalid @enderror" id="method_key" name="method_key" value="{{ old('method_key', $paymentMethod->method_key) }}" placeholder="lowercase_underscore_separated" required>
                            @error('method_key')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                ...
                                <option value="gateway" {{ old('type', $paymentMethod->type) == 'gateway' ? 'selected' : '' }}>Gateway</option>
                            </select>
                            @error('type')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('provider_name') is-invalid @enderror" id="provider_name" name="provider_name" value="{{ old('provider_name', $paymentMethod->provider_name) }}">
                            @error('provider_name')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                 ...
                 <div class="form-group" id="default_manual_account_group" style="{{ old('type', $paymentMethod->type) == 'manual' ? '' : 'display:none;' }}">
                    ...
                    <select class="form-select @error('default_manual_account_id') is-invalid @enderror" id="default_manual_account_id" name="default_manual_account_id">
                        ...
                        @foreach($manualPaymentAccounts as $account)
                            <option value="{{ $account->id }}" {{ old('default_manual_account_id', $paymentMethod->default_manual_account_id) == $account->id ? 'selected' : '' }}>
                 ...
                 <div class="row">
                    {{-- <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('logo_path') is-invalid @enderror" id="logo_path" name="logo_path" value="{{ old('logo_path', $paymentMethod->logo_path) }}">
                            @error('logo_path')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order" value="{{ old('sort_order', $paymentMethod->sort_order ?? 0) }}">
                            @error('sort_order')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
